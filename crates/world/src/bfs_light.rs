...
use crate::{BlockSource, CHUNK_HEIGHT, Chunk, ChunkCache, ChunkManager, ChunkManagerLike, Face};
...
impl<'a, C: ChunkManagerLike> BfsLight<'a, C> {
  ...
  pub fn remove(&mut self, node: LightNode) {
    ...
    self.chunk_manager.set_local_light(node.1, node.0, self.is_sky_light, if self.is_sky_light && node.0.y == 255 { 15 } else { 0 });
  }
      ...
      for face in [Face::Left, Face::Right, Face::Back, Face::Front, Face::Bottom, Face::Top] {
        ...
        if let Some(chunk) = self.chunk_manager.get_chunk_by_block_mut(neighbour_pos) {
          ...
          if local_position.y < CHUNK_HEIGHT {
            ...
            if (self.is_sky_light && face == Face::Bottom && node_light_level == 15) || (neighbour_light_level != 0 && neighbour_light_level < node_light_level)
            {
