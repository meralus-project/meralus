...
impl<T: Lerp + Clone> TypedTransition<T> {
    #[must_use]
    pub fn new(origin: T, destination: T, duration: u64, curve: Curve, repeat: RepeatMode) -> Self {
        Self {
            elapsed: 0.0,
            delay: 0.0,
            duration: Duration::from_millis(duration).as_secs_f32(),
    ...
    #[must_use]
    pub fn new_with_delay(origin: T, destination: T, duration: u64, delay: u64, curve: Curve, repeat: RepeatMode) -> Self {
        Self {
            elapsed: 0.0,
            delay: Duration::from_millis(delay).as_secs_f32(),
            duration: Duration::from_millis(duration).as_secs_f32(),
