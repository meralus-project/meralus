...
use meralus_shared::{Angle, Color, Cube3D, IPoint2D, IPoint3D, Point2D, Point3D, Ranged, Size3D, Transform3D, USizePoint3D, Vector2D, Vector3D};
use meralus_storage::Block;
use meralus_world::{
  BfsLight, BlockSource, CHUNK_HEIGHT, CHUNK_HEIGHT_F32, Chunk, ChunkCache, ChunkGenerator, ChunkManager, ChunkManagerLike, ChunkStage, Face, LightNode,
  LocalChunkManager, SUBCHUNK_COUNT, SUBCHUNK_COUNT_I32, SUBCHUNK_SIZE, SUBCHUNK_SIZE_F32, SUBCHUNK_SIZE_I32, SubChunk,
};
use tracing::info;

use crate::{
  AabbProvider, Camera, Debugging, FIXED_FRAMERATE, INVENTORY_HOTBAR_SLOTS, Interval, Item, LimitedAabbProvider, PlayerController, ResourceStorage, TICK_RATE,
  TPS, clock::Clock, cube_outline, input::Input, player::ItemType, vertex_ao,
};
...
impl ChunkCache for ChunkFileCache {
  fn all(&self) -> impl Iterator<Item = Chunk> {
    self.root.read_dir().into_iter().flatten().flat_map(|e| {
      let e = e.ok()?;
      let path = e.path();
  ...
  pub fn update(&mut self, white_pixel_uv: Point2D, debugging: &mut Debugging) {
    ...
    match self.chunk_receiver.try_recv() {
      Ok(chunk) => {
        ...
        // if chunk.subchunks != Chunk::empty().subchunks {
        ...
        // self.chunk_manager).apply_to_sky_light();
        ...
        //       if pos.y > 0
        //         && chunk
        //           .get_block(pos - USizePoint3D::Y)
        //           .is_some_and(|block|
        // self.resource_storage.blocks_light(block))       {
        //         bfs_light.addition_queue.push_back(LightNode(pos, origin));
        ...
        // info!(target: "client/world", ?origin, "Chunk light calculacted in
        // {:?}", instant.elapsed());

        // ChunkGenerator::new(self.seed.into()).populate(&mut
        // self.chunk_manager, self.resource_storage.as_ref(), self.seed.into(),
        // origin);

        // {
        //   let snapshot = WorldSnapshot::new(
        //     self.chunk_manager.get_chunk(origin).unwrap().clone(),
        //     &self.chunk_manager,
        //     self.resource_storage.clone(),
        //   );

        //   let subchunk_mesh_sender = self.subchunk_mesh_sender.clone();

        //   rayon::spawn(move || {
        //     let instant = Instant::now();

        //     for subchunk_idx in (0..SUBCHUNK_COUNT).rev() {
        //       if snapshot.chunk.subchunks[subchunk_idx].blocks.iter().
        // any(|block| block != &0) {         let mesh =
        // snapshot.compute_subchunk_mesh(subchunk_idx, None);

        //         subchunk_mesh_sender.send((origin, subchunk_idx,
        // mesh)).unwrap();       } else {
        //         subchunk_mesh_sender.send((origin, subchunk_idx, [Vec::new(),
        // Vec::new()])).unwrap();       }
        //     }

        //     info!(target: "client/world", origin = ?origin, "Chunk meshed in
        // {:?}", instant.elapsed());   });
        // }
...
impl<'a, C: ChunkManagerLike> WorldSnapshot<'a, C> {
  fn new(chunk_manager: &'a C, resource_storage: Arc<ResourceStorage>) -> Self {
    Self {
      chunk_manager,
      resource_storage,
    }
  ...
  pub fn compute_subchunk_mesh(&self, origin: IPoint2D, subchunk_idx: usize, #[allow(unused_variables)] marked: Option<IPoint3D>) -> [Vec<VoxelFace>; 2] {
    ...
    for (local_position, block_id) in subchunk.iter(subchunk_idx) {
      if let Some((block_id, model)) = block_id.map(|block_id| (block_id, self.resource_storage.models.get_unchecked(block_id.into()))) {
        ...
        let neighbours: [Option<u8>; 6] = Face::NORMALS.map(|face: IPoint3D| self.chunk_manager.get_block(world_position + face).filter(|b: &u8| *b != 0));
