...
use meralus_shared::{
  Angle, Color, Cube3D, IPoint2D, IPoint3D, Lerp, MatrixExt, Point2, Point2D, Point3D, Quat, RRect2D, Rect2D, Size2, Size2D, Thickness, Transform3D, USize2D,
  USizePoint3D, Vector2D, Vector3D,
};
...
use crate::{
  blocks::{
    AirBlock, BlueRoseBlock, BricksBlock, CobbleStoneBlock, DebugBlock, DirtBlock, GrassBlock, GreenGlassBlock, IceBlock, OakLeavesBlock, RoseBlock, SandBlock,
    SnowBlock, StoneBlock, StoneBricksBlock, TorchBlock, WaterBlock, WoodBlock,
  },
  camera::Camera,
  input::Input,
...
fn get_sky_color((after_day, progress): (bool, f32), weather: f32) -> Color {
  let day_color: Color = Color::from_hsl(220.0, 0.5 + 0.2 * weather, 0.75 - 0.6 * weather);
  let night_color: Color = Color::from_hsl(220.0, 0.35 + 0.1 * weather, 0.25 - 0.15 * weather);
        ...
        context.bounds(Rect2D::new(center, size), |context, _| {
          ...
          context.padding(2.0, |context, bounds| {
            context.clipped(bounds, |context, bounds| {
              ...
              let tile_gap = 2f32;
              let tile_size = (inner_size - Size2D::new((tile_count as f32 - 1.0) * tile_gap, (tile_count as f32 - 1.0) * tile_gap)) / tile_count as f32;
          ...
          world
            ...
            .and_then(
              |result| world
                ...
                .get_block(result.position)
                .filter(|&b| b != 0)
                .and_then(|block| self.resource_manager.get_block(block.into()).map(|block| format!(
                  "{} (at {}, sky light: {})",
                  block.id(),
                  result.hit_side,
