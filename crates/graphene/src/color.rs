...
#[repr(C)]
#[derive(Debug, Default, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, bytemuck::Pod, bytemuck::Zeroable)]
...
impl From<Color> for [f32; 4] {
    fn from(val: Color) -> Self {
        [
            f32::from(val.0[0]) / 255.0,
            f32::from(val.0[1]) / 255.0,
            f32::from(val.0[2]) / 255.0,
            f32::from(val.0[3]) / 255.0,
        ]
    }
}
...
impl From<Color> for [f32; 3] {
    fn from(val: Color) -> Self {
        [f32::from(val.0[0]) / 255.0, f32::from(val.0[1]) / 255.0, f32::from(val.0[2]) / 255.0]
    }
}

impl From<Color> for Vec4 {
    fn from(val: Color) -> Self {
        Self::from_array(val.into())
    }
}
